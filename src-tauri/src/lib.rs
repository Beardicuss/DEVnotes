...
async fn wait_oauth_callback() -> Result<serde_json::Value, String> {
    ...
    for p in query.split('&') {
        if let Some(v) = p.strip_prefix("code=") {
            code = v.to_string();
        }
        if let Some(v) = p.strip_prefix("state=") {
            state = v.to_string();
        }
    }
        ...
        loop {
            ...
            if changed {
                let _ = app.emit(
                    ...
                    serde_json::json!({ "projectId": pid, "path": dir }),
                );
            }
            *prev = current;
        }
...
#[tauri::command]
async fn stop_watching(_dir_path: String) -> Result<(), String> {
    Ok(())
}
...
async fn unregister_global_hotkey(app: tauri::AppHandle) -> Result<(), String> {
    ...
    app.global_shortcut()
        .unregister("Ctrl+Shift+D")
        .map_err(|e| format!("unreg: {e}"))
}
